...
func (c *Controller) updateCspStatus(spcGot *apis.StoragePoolClaim) {
	...
	for _, csp := range cspList.Items {
		...
		// cspPatch is the object of Patcher interface type
		var cspPatch patch.Patcher
		// Get the patch payload by passing the required arguments to the constructor.
		cspPatchPayload := patch.NewPatchPayload("add", "/status/nodeStatus", nodePhasePayload)
		// For object of PatchPayloadCSP to call the implementation of patch function for CSP.
		cspPatch = &patch.PatchPayloadCSP{
			Object:          &csp,
			PatchPayloadCSP: cspPatchPayload,
		}
